...
class User(Item):
    ...
    async def signin(self, email="", password=""):
        ...
        result = await self.do_request("POST", "signin", json=payload)
    ...
    async def verify_otp(self, email="", challenge="", code=""):
        ...
        result = await self.do_request("POST", "mfa/otp/validate", json=payload)
    ...
    async def get_profile(self):
        ...
        """
        result = await self.do_request("GET", "users/me")
        result = self.parent.element(result)
        return result
    ...
    async def promote_as_admin(self):
        """
        ...
        await domain.add_user(self._key)

    async def promote_as_super_admin(self):
        """
        ...
        return await domain.create_permission(self._key, "rwsadtlug", False)
    ...
    async def demote(self):
        ...
        try:
            await domain.remove_user(self._key)
            await domain.remove_participant(self._key)
        except Exception:
            pass
    ...
    async def get_admin_status(self):
        ...
        domain = self.parent.usergroup('domain')
        members = await domain.get_users()

        if (any([m for m in members if m._key == self._key])):
                status = 'admin'

        if status is not None:
            domain_permissions = await domain.get_permissions()
            if (any([p for p in domain_permissions if p.user._key == self._key])):
                status = 'super_admin'

        return status

    async def forgot_password(self, aquarium_url=None):
        """
